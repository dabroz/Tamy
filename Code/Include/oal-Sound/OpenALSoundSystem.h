...
/**
 * This class represents the OpenAL sound system- it encapsulates
 * all the methods it has that allow us to play sound
 */
class OpenALSoundSystem
{
private:
   HINSTANCE m_openALDLL;

public:
   OpenALSoundSystem(const std::string& openAlDllFile = "openal32.dll");
   ~OpenALSoundSystem();

   OALSoundDevice* createDevice(const SoundDeviceInfo& deviceDesc);
